...
def download_zenodo_record(
    zenodo_id: str, target_dir: str, force_download: bool = False, temp_dir: None | str = None
):
    import shutil
    import tempfile
    ...
    try:
        ...
        logger.error(
            f"Remove {target_zip_path} and retry, or download manually using `download_data.sh`."
        )
        raise e
